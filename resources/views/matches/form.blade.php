...
<div class="row g-3 mb-3">
    ...
    <div class="col-md-6">
        ...
        <input type="datetime-local" name="match_date" class="form-control" value="{{ old('match_date', isset($match) ? $match->match_date->format('Y-m-d\TH:i') : '') }}" required>
    </div>
    <div class="col-md-6">
    ...
    <div class="col-md-6">
        ...
        <select name="status" class="form-select">
            @foreach(['upcoming','live','completed'] as $st)
                <option value="{{ $st }}" {{ old('status', $match->status ?? 'upcoming') == $st ? 'selected' : '' }}>{{ ucfirst($st) }}</option>
            @endforeach
        </select>
    </div>
</div>